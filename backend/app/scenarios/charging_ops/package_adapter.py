...
from app.models.business import ChargingSession, DataGenerationRun
...
def semantic_definition(package: ScenarioPackage) -> dict:
    ...
    for item in dimensions["dimensions"]:
        normalized_dimensions.append({
            **item,
            "permission_policy": {"scope": "authorized_station_scope"},
...
def install_platform_foundation(
    db: Session,
    identity: IdentityContext,
    *,
    scenario_root: str | None = None,
) -> dict:
    package = ScenarioPackageLoader(scenario_root).load("charging_ops")
    scenario_registry = ScenarioRegistry(db)
    scenario_registry.validate(identity, package, ScenarioValidator())
    scenario_registry.publish(identity, package.scenario_id, package.version)
    scenario_release = scenario_registry.activate(
        identity, package.scenario_id, package.version
    )
    ...
    version = db.scalar(select(DatasetVersion).where(
        ...
        DatasetVersion.idempotency_key == f"{package.scenario_id}:{package.version}:dataset",
    ))
    if version is None:
        latest_batch = db.scalar(select(DataGenerationRun).where(
        ...
        version = dataset_service.create_version(
            ...
            source_version=latest_batch.batch_id if latest_batch else "simulated-seed-unavailable",
            ...
            compatible_semantic_range=">=0.1.0,<1.0.0",
