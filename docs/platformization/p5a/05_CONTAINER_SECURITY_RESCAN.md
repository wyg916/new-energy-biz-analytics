...
`IMAGE_SECURITY=BLOCKED`。Trivy 0.70.0 于 2026-08-03 刷新漏洞数据库后，对 11 个运行/禁用角色对应的 9 个唯一 image ID 完成实际全镜像漏洞扫描；没有 ignore、severity 降级、结果删除或虚构 waiver。

按唯一 image ID 计为 50 个 Critical 实例、834 个 High 实例、24 个唯一 Critical 标识、642 个唯一 High 标识；按 11 个角色累计为 51 Critical、848 High，因为 backup 与 PostgreSQL 使用同一 image ID。无正式 waiver，任何 Critical/High 均未标为 ACCEPTED。

## 镜像与结果

| 角色 | 镜像/固定引用 | 内容 digest 或 image ID | C | H | 唯一 CVE/漏洞标识 | 状态 |
|---|---|---|---:|---:|---:|---|
| API | `renewable-p5a-api:5.0.0-p5a` | `sha256:d413e1427d5b470b862a8bc21800ea053f34c7435f2f0d91dff9f9c037a0b432` | 0 | 0 | 1 | PASS |
...
| SQLBot（禁用且排除 RC） | `dataease/sqlbot:v1.8.0` | `sha256:c4ca3acc34f0c63a64f3f3e7bb909760f17d184959635542278710144347d9e0` | 49 | 802 | 3595 | BLOCKED |
| 告警接收端 | `renewable-p5a-alert-receiver:5.0.0-p5a` | `sha256:42abcf52fdc4162f83bd4b188928e0d11a6b6afa9225eea6efff7dbd1a448205` | 0 | 0 | 1 | PASS |
| migration | `renewable-p5a-migration:5.0.0-p5a` | 与告警接收端 image ID 相同 | 0 | 0 | 1 | PASS |
| backup | `renewable-p5a-backup:16.14` | 与 PostgreSQL image ID 相同 | 1 | 14 | 39 | BLOCKED |

“唯一 CVE/漏洞标识”包含所有 severity，不等同于 C+H。详细 affected package、installed/fixed version、扫描完成时间、Trivy 版本、原始 JSON 文件名、是否进入 RC 和原始证据 SHA-256 均在 `evidence/container-security/container-security-summary.json`。每个角色另有 `trivy-<role>.json`；相同 image ID 的角色使用相同字节证据，并由 `scan_execution=REUSED_IDENTICAL_IMAGE_ID` 明确标注，没有重复声称为独立内容扫描。
...
- SQLBot 本轮仍完整扫描为 49C/802H，但 `included_in_rc=false`；扫描没有启动 SQLBot 服务、外部模型或 Canary。
