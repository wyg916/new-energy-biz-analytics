...
type Snapshot = {
  ...
  runtime: { readiness_http_status: number; readiness: Record<string, unknown>; query_engine_mode: string; sqlbot_engine_enabled: boolean; sqlbot_canary_eligible: boolean; production_release_enabled: boolean; data_classification: string }
}
...
export function GovernancePage({ token }: { token: string }) {
  ...
  return <div className="governance-page" data-testid="governance-page">
    ...
    {tab === 'credentials' && <section className="gov-panel"><header><div><h3>CredentialReference</h3><p>只显示引用元数据、用途、作用域和状态；不会显示 Secret 值。</p></div><button disabled title="请通过受控管理 API 创建凭据引用">创建由管理 API 控制</button></header>{snapshot.credentials.length ? <table><thead><tr><th>引用</th><th>Provider / 环境</th><th>用途</th><th>允许动作</th><th>版本</th><th>状态</th><th>操作</th></tr></thead><tbody>{snapshot.credentials.map(item => <tr key={item.credential_ref_id}><td><b>{item.reference_name}</b><small>{item.credential_ref_id}</small></td><td>{item.provider} / {item.environment}</td><td>{item.purpose}</td><td>{item.allowed_actions.join(', ')}</td><td>v{item.version}</td><td><Status value={item.status} /></td><td><button disabled={item.status !== 'ACTIVE'} title={item.status !== 'ACTIVE' ? '凭据引用已非 ACTIVE' : '停用会立即撤销该运行时引用'} onClick={() => void action(`/api/v1/governance/credentials/${item.credential_ref_id}/disable`)}>禁用</button></td></tr>)}</tbody></table> : <Empty>尚未创建 CredentialReference；SQLBot 外部复评保持 CONDITIONAL，并在外部请求前 fail-closed。</Empty>}<footer className="gov-boundary">Secret value exposed: <b>0</b> · 自动扫描 .env*: <b>禁止</b> · 缺失 Provider: <b>fail-closed</b></footer></section>}
