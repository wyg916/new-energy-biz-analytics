...
const nav = ['经营总览','收入分析','毛利分析','场站分析','设备分析','可信问数']
...
function ChatPanel({token}:{token:string}){
  const[question,setQuestion]=useState('区域A在2026年6月充电收入和毛利率是多少？');const[response,setResponse]=useState<any>(null);const[loading,setLoading]=useState(false);const[error,setError]=useState('')
  async function ask(event:React.FormEvent){event.preventDefault();setLoading(true);setError('');try{const r=await fetch('/api/v1/chat/query',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({question})});const body=await r.json();if(!r.ok)throw new Error(body.detail?.message||'问数失败');setResponse(body)}catch(e){setError(e instanceof Error?e.message:'问数失败')}finally{setLoading(false)}}
  return <section className="chat-layout"><article className="panel chat-main"><div className="panel-title"><div><h2>可信 ChatBI</h2><p>自然语言 → Query Plan → 安全编译 → 只读执行 → Answer Guard</p></div><span className="safe-chip">受控查询</span></div><form className="question-box" onSubmit={ask}><textarea value={question} onChange={e=>setQuestion(e.target.value)} maxLength={500}/><button disabled={loading}>{loading?'正在验证并计算…':'开始分析'}</button></form>{error&&<div className="notice error">{error}</div>}{response&&<div className="answer"><span className={`status ${response.status}`}>{response.status}</span><h3>回答</h3><p>{response.answer}</p>{response.chart&&<div className="chart-data">图表：{response.chart.type} · {response.chart.metric_id} · {response.chart.data.length} 个数据点</div>}<details><summary>查看 Query Plan</summary><pre>{JSON.stringify(response.query_plan,null,2)}</pre></details></div>}</article>{response&&<aside className="panel evidence-panel"><h2>证据面板</h2><dl><div><dt>数据分类</dt><dd>{response.evidence.data_classification}</dd></div><div><dt>来源</dt><dd>{response.evidence.source}</dd></div><div><dt>批次</dt><dd>{response.evidence.batch_id||'未执行'}</dd></div><div><dt>分析运行</dt><dd>{response.evidence.analysis_run_id}</dd></div><div><dt>Query Guard</dt><dd>{response.evidence.query_guard}</dd></div><div><dt>Answer Guard</dt><dd>{response.evidence.answer_guard?.status||'未执行'}</dd></div><div><dt>SQL hash</dt><dd>{response.evidence.sql_hash?.slice(0,16)||'未生成'}</dd></div><div><dt>解释方式</dt><dd>{response.evidence.explanation_mode}</dd></div></dl><details><summary>分析师 SQL 证据</summary><pre>{response.evidence.sql||'当前角色不显示 SQL 或本次未执行'}</pre></details></aside>}</section>
}
...
function Dashboard({token,onLogout}:{token:string;onLogout:()=>void}){
  ...
  const primary=active==='可信问数'?'charging_revenue':navMetrics[active][0]
  ...
  useEffect(()=>{if(active!=='可信问数')void load()},[active])
  ...
  return <div className="app-shell">
    ...
    <main><header><div><h1>{active}</h1><p>统一口径 · 权限过滤 · 可审计证据</p></div><div className="toolbar"><input type="date" value={start} onChange={e=>setStart(e.target.value)}/><span>至</span><input type="date" value={end} onChange={e=>setEnd(e.target.value)}/><button onClick={()=>void load()}>刷新</button><button className="ghost" onClick={onLogout}>退出</button></div></header>
      {active==='可信问数'?<ChatPanel token={token}/>:<>{error&&<div className="notice error">{error}</div>}{loading&&<div className="notice">正在从平台数据库计算指标…</div>}
