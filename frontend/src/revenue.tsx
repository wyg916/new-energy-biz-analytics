...
export function RevenuePage({ token, summary, stations, start, end, setStart, setEnd, refresh }: RevenuePageProps) {
  ...
  return <div className="revenue-page">
    ...
    <section className="revenue-bottom-grid">
      ...
      <article className="revenue-panel revenue-insight-panel"><header><div><h2>结构化经营洞察</h2><p>仅引用本页结构化结果</p></div><span>规则生成</span></header><ol>{insights.map((item, index) => <li key={item}><i>{index + 1}</i><p>{item}</p></li>)}</ol><button disabled>依据为本页结构化结果</button></article>
    </section>
