...
RESTORE_NAME = re.compile(r"^p5a_restore_verify_[a-z0-9_]{4,40}$")

INNER_SCRIPT = r'''
set -eu
restore_db="$1"
stamp="$2"
backup_file="/backups/p5a-acceptance-$stamp.dump"
case "$restore_db" in
  p5a_restore_verify_*) ;;
...
trap cleanup EXIT
pg_dump -h db -U alpha -d renewable_p5a --format=custom --no-owner --file="$backup_file"
...
source_summary="$(psql -h db -U alpha -d renewable_p5a -At -v ON_ERROR_STOP=1 -c "$summary_sql")"
restore_summary="$(psql -h db -U alpha -d "$restore_db" -At -v ON_ERROR_STOP=1 -c "$summary_sql")"
source_hash="$(psql -h db -U alpha -d renewable_p5a -At -v ON_ERROR_STOP=1 -c "$hash_sql")"
...
def main() -> None:
    ...
    if not RESTORE_NAME.fullmatch(args.restore_database):
        parser.error("restore database must use the p5a_restore_verify_ prefix")
    started_at = datetime.now(UTC)
    started_monotonic = time.monotonic()
    stamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    process = subprocess.run(
        ["docker", "exec", "-i", args.container, "sh", "-s", "--", args.restore_database, stamp],
    ...
    payload.update({
        "evidence_type": "p5a_backup_restore_drill",
